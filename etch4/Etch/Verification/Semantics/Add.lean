...
def Stream.add [Zero α] [Add α] (a b : Stream ι α) : Stream ι α
  ...
  skip p hp i := (a.skip' p.1 i, b.skip' p.2 i)
  index s h := Option.get (show (min (a.index' s.1) (b.index' s.2)).isSome by simpa)
  value s h :=
...
theorem add_index_eq_min {a b : Stream ι α} (q : (a.add b).σ) :
  ...
  · simp [H]
  · simp [H, not_or_distrib.mp H]
#align add_index_eq_min Etch.Verification.add_index_eq_min

theorem valid_of_to_order_lt {a b : Stream ι α} {qa : a.σ} {qb : b.σ}
    (h : a.toOrder' qa < b.toOrder' qb) : a.valid qa :=
  by
  contrapose! h
...
#align valid_of_to_order_lt Etch.Verification.valid_of_to_order_lt
...
theorem of_to_order_eq {a b : Stream ι α} (q : (a.add b).σ) (h : a.toOrder' q.1 = b.toOrder' q.2) :
    ¬(a.add b).valid q ∧ ¬a.valid q.1 ∧ ¬b.valid q.2 ∨
      (a.add b).valid q ∧
        a.valid q.1 ∧ b.valid q.2 ∧ (a.Ready q.1 ↔ b.Ready q.2) ∧ a.index' q.1 = b.index' q.2 :=
  by
  ...
  · left
    refine' ⟨H, not_or_distrib.mp H⟩
#align of_to_order_eq Etch.Verification.of_to_order_eq

theorem add_to_order_eq_min {a b : Stream ι α} (q : (a.add b).σ) :
    (a.add b).toOrder' q = min (a.toOrder' q.1) (b.toOrder' q.2) :=
  by
  ...
  · rw [min_eq_right h.le]
    ...
    · simpa using Prod.Lex.fst_le_of_le h.le
    · simp [valid_of_to_order_lt h, h.le, h.not_le]
#align add_to_order_eq_min Etch.Verification.add_to_order_eq_min
...
theorem add_toOrder_left {a b : Stream ι α} {q : (a.add b).σ} (hq hq')
  ...
  coeLex_injective (by simpa [add_to_order_eq_min] )
...
instance Stream.add.isBounded (a b : Stream ι α) [IsBounded a] [IsBounded b] :
    IsBounded (a.add b) :=
  IsBounded.mk'
    ⟨Prod.RprodEq a.WfRel b.WfRel, Prod.rprodEq_wf a.wf b.wf, fun q i =>
      by
      ...
      right; constructor
      · rw [add_to_order_eq_min, lt_min_iff]
        constructor <;> assumption; · ext <;> simpa⟩
...
theorem add_strict_mono {a b : Stream ι α} (ha : a.IsStrictMono) (hb : b.IsStrictMono) :
  ...
  ⟨add_mono ha.1 hb.1, fun q hq i hi hr =>
    ne_of_lt
      (by
        replace hi : min (a.to_order' q.1) (b.to_order' q.2) ≤ coeLex i;
        · rwa [← add_to_order_eq_min, ← Stream.coeLex_toOrder hq, coeLex_le_iff]
        rcases lt_trichotomy (a.to_order' q.1) (b.to_order' q.2) with (h | h | h)
        · replace hr : a.ready q.1 := by simpa [h.le, h.not_le] using hr
          have hqa : a.valid q.1 := valid_of_to_order_lt h
          replace hi : a.to_order q.1 hqa ≤ i
        ...
        · obtain ⟨hv₀, hv₁, hv₂, hr_iff, hind⟩ :=
            (of_to_order_eq _ h).resolve_left fun h' => h'.1 hq
        ...
        · replace hr : b.ready q.2 := by simpa [h.le, h.not_le] using hr
          ...
          replace hi : b.to_order q.2 hqb ≤ i
...
theorem Stream.add_map {β : Type _} [Zero β] [Add β] (f : α → β)
    ...
    (q.add r).map f = (q.map f).add (r.map f) :=
  by
  ext <;>
    trace
      "./././Mathport/Syntax/Translate/Tactic/Builtin.lean:73:14: unsupported tactic `solve_refl #[]"
...
theorem Stream.add.eval₀_eq_left {a b : Stream ι α} {q : (a.add b).σ} (hq : (a.add b).valid q)
  ...
  by
  have := add_toOrder_left hq (valid_of_to_order_lt H) H.le
  have hr : (a.add b).Ready q ↔ a.ready q.1 := by simpa using congr_arg Prod.snd this
...
theorem Stream.add.eval₀_eq_right {a b : Stream ι α} {q : (a.add b).σ} (hq : (a.add b).valid q)
  ...
  by
  have := add_toOrder_right hq (valid_of_to_order_lt H) H.le
  have hr : (a.add b).Ready q ↔ b.ready q.2 := by simpa using congr_arg Prod.snd this
  simp only [Stream.eval₀, ← Stream.value'_val, dite_eq_ite]
  rw [hr]
  split_ifs with hr'
  swap
...
theorem Stream.add.eval₀_eq_both {a b : Stream ι α} {q : (a.add b).σ} (hq : (a.add b).valid q)
    ...
    (a.add b).eval₀ q hq = a.eval₀ q.1 hv₁ + b.eval₀ q.2 hv₂ :=
  by
...
theorem add_spec (a b : Stream ι α) [IsLawful a] [IsLawful b] (q : (a.add b).σ) :
  ...
  by
  refine' @WellFounded.induction _ (a.add b).WfRel (a.add b).wf _ q _
  clear q; intro q ih
  rcases lt_trichotomy (a.to_order' q.1) (b.to_order' q.2) with (h | h | h)
  · have hv : a.valid q.1 := valid_of_to_order_lt h
    ...
    rw [(a.add b).eval_valid q hq, ih _ ((a.add b).next_wf _ hq), Stream.next_val hq,
      Stream.add_skip, @Stream.skip'_lt_to_order _ _ _ _ b, add_toOrder_left hq hv h.le,
  ...
  · rcases of_to_order_eq _ h with (⟨H, hv₁, hv₂⟩ | ⟨hq, hv₁, hv₂, hr, hi⟩)
    · simp [H, hv₁, hv₂]
    obtain ⟨t₀, t₁⟩ :
      (a.add b).toOrder q hq = a.to_order q.1 hv₁ ∧ (a.add b).toOrder q hq = b.to_order q.2 hv₂ :=
      by constructor <;> apply_fun coeLex using coeLex_injective <;> simp [add_to_order_eq_min, h]
  ...
  · have hv : b.valid q.2 := valid_of_to_order_lt h
    ...
    rw [(a.add b).eval_valid q hq, ih _ ((a.add b).next_wf _ hq), Stream.next_val hq,
      Stream.add_skip, @Stream.skip'_lt_to_order _ _ _ _ a, add_toOrder_right hq hv h.le,
...
instance (a b : Stream ι α) [IsLawful a] [IsLawful b] : IsLawful (a.add b) :=
  ⟨add_mono a.mono b.mono, fun q hq i j hj =>
    by
    simp only [add_spec]; dsimp
    congr 1 <;> rwa [Stream.skip'_spec]⟩

instance (a b : Stream ι α) [IsStrictLawful a] [IsStrictLawful b] : IsStrictLawful (a.add b)
    where StrictMono := add_strict_mono a.StrictMono b.StrictMono
