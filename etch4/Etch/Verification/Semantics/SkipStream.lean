...
theorem Stream.ext {ι α} {s₁ s₂ : Stream ι α} (h₀ : s₁.σ = s₂.σ)
    ...
    (h₅ : ∀ x y H₁ H₂, HEq x y → HEq (s₁.value x H₁) (s₂.value y H₂)) : s₁ = s₂ :=
  by
  ...
  refine' ⟨rfl, rfl, rfl, _, _, _⟩ <;> try simp only [heq_iff_eq] at * <;> ext;
  · apply h₃ x x _ _ _ rfl <;> assumption; · apply h₄ x x _ _ rfl <;> assumption
...
theorem Stream.coeLex_toOrder {s : Stream ι α} {q : s.σ} (hq : s.valid q) :
    coeLex (s.toOrder q hq) = s.toOrder' q := by simp [Stream.toOrder'_val, hq]
...
theorem Stream.next_wf (s : Stream ι α) [IsBounded s] (q) (hq : s.valid q) : s.next q ≺ q :=
  by
...
theorem Stream.eval₀_support' [Zero α] (s : Stream ι α) {x : s.σ} (h₁ : s.valid x) {i : ι}
    (h₂ : s.eval₀ x h₁ i ≠ 0) : s.toOrder x h₁ = (i, true) :=
  by
  ...
  · simpa [Stream.toOrder, Stream.index'_val h₁] ; · simpa using h₂
#align Stream.eval₀_support' Etch.Verification.Stream.eval₀_support'
...
theorem Stream.isMonotonic_iff {s : Stream ι α} :
    s.IsMonotonic ↔ ∀ q hq i hq', s.index q hq ≤ s.index (s.skip q hq i) hq' :=
  by
  constructor;
  · intro h q hq i hq'
    specialize h q hq i
    simp only [Stream.index'_val, hq, hq', WithTop.coe_le_coe] at h
    exact h
  intro h q hq i
  by_cases hq' : s.valid (s.skip q hq i)
  · simp only [Stream.index'_val, hq, hq', WithTop.coe_le_coe]
    apply h
...
theorem Stream.IsMonotonic.index_le_of_mem_support [AddZeroClass α] {s : Stream ι α} [IsBounded s]
    (hs : s.IsMonotonic) {q : s.σ} : ∀ i : ι, s.eval q i ≠ 0 → s.index' q ≤ i :=
  by
  simp only [← Finsupp.mem_support_iff]; refine' WellFounded.induction s.wf q _
  intro q ih i hi
...
theorem Stream.IsMonotonic.eq_zero_of_lt_index [AddZeroClass α] {s : Stream ι α} [IsBounded s]
    (hs : s.IsMonotonic) {q : s.σ} (i : ι) : ↑i < s.index' q → s.eval q i = 0 :=
  by
  contrapose!
...
theorem Stream.IsStrictMono.next_ne {s : Stream ι α} (hs : s.IsStrictMono) {q : s.σ}
    (hq : s.valid q) (hr : s.ready q) : s.index' q ≠ s.index' (s.next q) :=
  by
  rw [Stream.next_val hq]
...
theorem Stream.IsStrictMono.eq_zero_of_lt_index [AddZeroClass α] {s : Stream ι α} [IsBounded s]
    ...
    ((j, false) <ₗ s.toOrder q hv) → s.eval (s.skip q hv i) j = 0 :=
  by
  contrapose!
  exact hs.index_le_of_mem_support hv hi j
...
theorem Stream.IsStrictMono.eval_skip_eq_zero [AddZeroClass α] {s : Stream ι α} [IsBounded s]
    ...
    (h₁ : (j, false) <ₗ i) (h₂ : i ≤ₗ s.toOrder q hv) : s.eval (s.skip q hv i) j = 0 :=
  by
...
theorem Stream.IsStrictMono.eval₀_eq_eval_filter [AddCommMonoid α] {s : Stream ι α} [IsBounded s]
    ...
    s.eval₀ q hv = (s.eval q).filter fun i => (i, false) <ₗ s.toOrder q hv :=
  by
  ...
  · rw [Finsupp.filter_eq_zero_iff]
    ...
    refine' hs.eq_zero_of_lt_index hv rfl.le i hi
...
theorem Stream.skip_spec (s : Stream ι α) [IsLawful s] (q : s.σ) (hq : s.valid q)
    ...
    ((s.eval (s.skip q hq i)).filter fun j => i ≤ₗ (j, false)) =
      (s.eval q).filter fun j => i ≤ₗ (j, false) :=
  by
...
theorem Stream.skip_lt_toOrder {s : Stream ι α} [IsLawful s] {q : s.σ} {hq : s.valid q}
    {i : StreamOrder ι} (hi : i < s.toOrder q hq) : s.eval (s.skip q hq i) = s.eval q :=
  by
  ext j
  ...
  have : ↑j < s.index' q := by
    simpa [hq] using fst_lt_of_lt_of_lt (show (j, ff) <ₗ (j, tt) by simp) (not_le.mp H)
...
theorem Stream.skip'_spec (s : Stream ι α) [IsLawful s] (q : s.σ) (i : StreamOrder ι) (j : ι)
    (h : i ≤ (j, false)) : s.eval (s.skip' q i) j = s.eval q j :=
  by
  by_cases hq : s.valid q
...
theorem Stream.skip'_lt_to_order {s : Stream ι α} [IsLawful s] {q : s.σ} {i : StreamOrder ι}
    (hi : coeLex i < s.toOrder' q) : s.eval (s.skip' q i) = s.eval q :=
  by
...
unsafe def tactic.interactive.solve_refl : tactic Unit :=
  let tactics : List (tactic Unit) := [sorry, sorry, sorry, sorry, sorry]
  sequence (tactics.map fun t => tactic.try t) >> tactic.skip
#align tactic.interactive.solve_refl Etch.Verification.tactic.interactive.solve_refl
...
@[simp]
theorem Stream.map_id (s : Stream ι α) : s.map id = s := by
  ext <;>
    trace
