...
-- (wf_valid : ∀ (q hq i), (wf_rel (s.skip q hq i) q) ∨ 
--   ((i <ₗ Stream.to_order _ q hq) ∧ (s.skip q hq i) = q))
...
lemma Stream.wf_valid (s : Stream ι α) [is_bounded s] :
  ∀ (q hq i), (s.wf_rel (s.skip q hq i) q) ∨ ((i <ₗ s.to_order q hq) ∧ (s.skip q hq i) = q) := ‹is_bounded s›.out.some_spec.2
