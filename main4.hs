...
import Prelude hiding (min, max, replicate, String, concat, and, repeat)
...
egmul1 = run $ down <$> (vvar <-- sum2 (m "A" <.> repl1 (v "x")))
egmul2 = run $ down2 <$> (mvar <-- sum3 (repl2 (m "A") <.> repl1 (m "B")))
